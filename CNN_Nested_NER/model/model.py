...
try:
    ...
    def scatter_max(src, index, dim=1):
        if dim != 1:
            raise NotImplementedError('Only dim=1 is required by CNNNer')
        expanded_index = index.unsqueeze(-1).expand_as(src)
        output = src.new_full((src.size(0), int(index.max().item()) + 1, src.size(2)), float('-inf'))
        output.scatter_reduce_(1, expanded_index, src, reduce='amax', include_self=True)
        used = torch.zeros((src.size(0), output.size(1)), dtype=torch.bool, device=src.device)
        used.scatter_(1, index, True)
        output.masked_fill_(~used.unsqueeze(-1), 0)
        return output, None
import torch
