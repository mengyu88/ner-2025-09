...
from fastNLP import TorchWarmupCallback, CheckpointCallback
import fitlog
# fitlog.debug()
...
parser.add_argument('--data_dir', default=None, type=str,
                    help='Directory containing train/dev/test.jsonlines for customized data.')
...
parser.add_argument('--save_dir', default='outputs/default', type=str)
...
fitlog.set_log_dir(os.path.join(args.save_dir, 'logs'))
...
def get_data(dataset_name, model_name, data_dir=None):
    # 以下是我们自己的数据
    if data_dir is not None:
        ...
        raise RuntimeError("Does not support. Use --data_dir for customized train/dev/test.jsonlines data.")
    pipe = SpanNerPipe(model_name=model_name)
    dl = pipe.process_from_file(paths)

    return dl, pipe.matrix_segs

dl, matrix_segs = get_data(dataset_name, model_name, args.data_dir, _refresh=args.refresh_cache)
def densify(x):
...
callbacks.append(CheckpointCallback(folder=os.path.join(args.save_dir, 'checkpoints'),
                                    topk=1,
                                    monitor='f#f#dev',
                                    larger_better=True,
