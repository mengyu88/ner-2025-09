    ...
    for epoch in train_iterator:
        ...
        if args.evaluate_during_training:
            ...
            if args.local_rank == 0 or args.local_rank == -1:
                ...
                metrics_file = os.path.join(args.output_dir, "epoch_metrics.csv")
                row = {
                    "epoch": epoch + 1,
                    "step": global_step,
                    "dev_acc": dev_metrics["acc"],
                    "dev_precision": dev_metrics["p"],
                    "dev_recall": dev_metrics["r"],
                    "dev_f1": dev_metrics["f1"],
                    "test_acc": test_metrics["acc"],
                    "test_precision": test_metrics["p"],
                    "test_recall": test_metrics["r"],
                    "test_f1": test_metrics["f1"],
                    "remark": "best" if is_best else "",
                }
...
def main():
    ...
    if torch.cuda.is_available() and args.local_rank != -1:
        ...
        args.n_gpu = 1 if device.type == "cuda" else 0
    #################################################
