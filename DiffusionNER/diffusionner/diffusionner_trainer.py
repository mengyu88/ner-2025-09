...
class DiffusionNERTrainer(BaseTrainer):
    ...
    def train(self, train_path: str, valid_path: str, types_path: str, input_reader_cls: BaseInputReader):
        ...
        # train
        if self.record:
            self._init_epoch_metrics_logging()
            ...
            if (not args.final_eval or (epoch == args.epochs - 1)) and self.record and ((epoch%args.eval_every_epochs)==0 or (epoch == args.epochs - 1)):
                ...
                if is_best:
                    ...
                    self._save_model(self._save_path, model, self._tokenizer, epoch * updates_epoch,
                            ...
                            include_iteration=False, save_as_best=True, name='model')
                self._log_epoch_metrics(epoch + 1, (epoch + 1) * updates_epoch, f1, test, is_best)
    ...
    def _init_epoch_metrics_logging(self):
        self._epoch_metrics_path = getattr(self.args, "epoch_metrics_path", None)
        if not self._epoch_metrics_path:
            return

        util.create_directories_file(self._epoch_metrics_path)
        with open(self._epoch_metrics_path, "w", newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow([
                "epoch", "step",
                "dev_entity_f1", "dev_entity_precision", "dev_entity_recall",
                "test_entity_f1", "test_entity_precision", "test_entity_recall",
                "remark"
            ])

    def _log_epoch_metrics(self, epoch, step, dev_metrics, test_metrics, is_best):
        if not getattr(self, "_epoch_metrics_path", None):
            return

        row = [
            epoch, step,
            f"{dev_metrics[2]:.2f}", f"{dev_metrics[0]:.2f}", f"{dev_metrics[1]:.2f}",
            "", "", "",
            "best" if is_best else ""
        ]
        if test_metrics is not None:
            row[5:8] = [f"{test_metrics[2]:.2f}", f"{test_metrics[0]:.2f}", f"{test_metrics[1]:.2f}"]

        with open(self._epoch_metrics_path, "a", newline="") as csv_file:
            writer = csv.writer(csv_file)
            writer.writerow(row)
