        ...
        while not run_args.cpu and (len(gpu_queue)==0 or len(gpu_queue)<run_args.world_size):
            ...
            if pynvml is None:
                if run_args.world_size > 0:
                    gpu_queue = [(index, 0) for index in all_gpu_queue[:run_args.world_size]]
                else:
                    gpu_queue = [(all_gpu_queue[0], 0)]
                print("pynvml unavailable; using devices: ", list(map(lambda x: x[0], gpu_queue)))
                break
