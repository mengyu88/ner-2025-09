#!/usr/bin/env bash
# GENIA: SNSA + HSR + relation-aware span classifier.
# Model selection is strictly based on development F1; test is logged only.
set -euo pipefail
...
exec env CUDA_VISIBLE_DEVICES=0 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  --use_snsa 1 --use_hsr 1 \
  --head_type relation_aware --relation_dim 192 \
