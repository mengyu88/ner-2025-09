#!/usr/bin/env bash
# GENIA: training-only boundary-span collaborative supervision on SPSR-Net.
# SNSA and HSR remain enabled; model selection uses development F1 only.
set -euo pipefail
...
exec env CUDA_VISIBLE_DEVICES=0 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  --boundary_aux_weight 0.2 --boundary_align_weight 0.05 \
