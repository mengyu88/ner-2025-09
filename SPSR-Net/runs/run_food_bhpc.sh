...
# A dev-selected, no-PGD BHPC trial. Do not use test metrics to select a
# checkpoint; evaluate the saved best-dev checkpoint only after training.
...
RUN_DIR="$PROJECT/runs/food_bhpc_$STAMP"

mkdir -p "$RUN_DIR"
printf '%s\n' "$RUN_DIR" > "$PROJECT/runs/latest_food_bhpc_dir.txt"
