#!/usr/bin/env bash
# GENIA: hierarchy-length difficulty curriculum on the existing ASL loss.
set -euo pipefail
...
exec env CUDA_VISIBLE_DEVICES=0 PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
  ...
  --difficulty_curriculum --curriculum_warmup_steps 7512 --curriculum_ramp_steps 11268 \
  --curriculum_nested_boost 0.4 --curriculum_long_boost 0.2 \
  --curriculum_hard_neg_boost 0.15 --curriculum_long_span 4 \
